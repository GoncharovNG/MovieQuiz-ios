...
//  MovieQuiz
//
//  Created by Никита Гончаров on 21.02.2023.
//
...
final class StatisticServiceImplementation: StatisticService {
    ...
    var gamesCount: Int {
        get {
            guard let data = userDefaults.data(forKey: Keys.gamesCount.rawValue), let gamesCount = try?
                    JSONDecoder().decode(Int.self, from: data) else {
                return .zero
            }
            return gamesCount
        }
    ...
    func store(correct count: Int, total amount: Int) {
        ...
        if let bestGame = userDefaults.data(forKey: Keys.bestGame.rawValue),
           let record = try? JSONDecoder().decode(GameRecord.self, from: bestGame) {
            if record.reviewRecord(currentCorrect: count, recordCorrect: record.correct) {
